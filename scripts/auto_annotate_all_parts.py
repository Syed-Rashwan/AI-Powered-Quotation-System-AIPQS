...
def main():
    for part_num in range(1, 6):
        image_dir = Path(f'datasets/part_{part_num}/train')
        label_dir = Path(f'datasets/part_{part_num}/labels')
        label_dir.mkdir(exist_ok=True)
        
        print(f'Annotating part_{part_num}...')
        for img_path in image_dir.glob('*.png'):
            label_path = label_dir / f'{img_path.stem}.txt'
            auto_annotate(str(img_path), str(label_path))
            print(f'Annotated {img_path.name} -> {label_path.name}')
        print(f'Completed annotation for part_{part_num}.')
